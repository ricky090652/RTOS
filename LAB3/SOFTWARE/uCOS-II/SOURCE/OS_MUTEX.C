...
INT8U  OSMutexPost (OS_EVENT *pevent)
{
    ...
    if (pevent->OSEventGrp != 0x00) {                 /* Any task waiting for the mutex?               */
        ...
        if (prio != pip) {
            ...
            if (CtxSwMessageTop < CtxSwMessageSize) {
            if(TimeStart)
                sprintf(CtxSwMessage[CtxSwMessageTop++],"%5d lock R%d  (Prio=%d changes to =%d)\n", (int)OSTime,(int)pip, (int)OSTCBCur->OSTCBPrio, (int)pip);
        }
        }
